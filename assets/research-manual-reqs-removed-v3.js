...
function F(l,g,t){
  return t==="unit-special-training"?l.maxLevel:Math.min(l.maxLevel,g.maxLevel)
}

function unlockGateLevel(l,g,t){
  return l==="unit-special-training"?g.maxLevel:Math.min(g.maxLevel,t+1)
}
...
function ie(l,g={},t={}){
  const m={};
  for(const r of l){
    const u=g[r.id]||{},h=t[r.id]||{},d=(u.completedLevels??0)>0||(u.completed??0)>0||(u.total??0)>0&&u.total-u.remaining>0||Object.values(h).some(o=>Number(o)>0);
    if(!d)continue;
    for(const o of r.unlockRequirements||[])m[G(String(o))]=!0
  }
  return m
}
  ...
  for(const u of l){
    ...
    for(const n of u.nodes){
      ...
      const b=(n.parents||[]).every(s=>{
        const a=d.get(s);
        return a&&(h[s]||0)>=unlockGateLevel(u.id,a,x)
      });
      if(!b)continue;
      const c=[],j=J(n,x,m);
      ...
      for(const s of o.get(n.id)||[]){
        const a=d.get(s),i=a?F(n,a,u.id):n.maxLevel;
        if(x<i&&x+1<=i){
          const Q=f(s);
  ...
  for(const u of l){
    ...
    for(const c of u.nodes){
      ...
      const b=(c.parents||[]).every(j=>{
        const A=d.get(j);
        return A&&(h[j]||0)>=unlockGateLevel(u.id,A,x)
      });
      if(!b)continue;
      const j=[],A=J(c,x,m);
      ...
      for(const q of o.get(c.id)||[]){
        const z=d.get(q),$=z?F(c,z,u.id):c.maxLevel;
        if(x<$&&x+1<=$){
          const Z=f(q);
  ...
  for(const r of l){
    ...
    for(const d of r.nodes){
      ...
      const b=(d.parents||[]).every(c=>{
        const x=h.get(c);
        return x&&(u[c]||0)>=unlockGateLevel(r.id,x,o)
      });
...
function D({trees:l,completions:g,summaries:s={}}){
  const{getStorageKey:a}=R(),n=a("RESEARCH_TROOP_FOCUS"),x=a("RESEARCH_BADGE_COUNT"),y=a("RESEARCH_RECOMMENDATION_MODE"),[p,z]=S(n,V.map(s=>s.id)),[t,m]=S(x,""),[mode,setMode]=S(y,"plans"),r=N.useMemo(()=>new Set(Array.isArray(p)?p:V.map(s=>s.id)),[p]),A=N.useMemo(()=>ie(l,s,g),[l,s,g]),h=N.useMemo(()=>Math.max(0,parseInt(String(t).replace(/[^\d]/g,""),10)||0),[t]),d=N.useMemo(()=>h>0?X(l,h,g,r,s,A):[],[l,h,g,r,s,A]),plans=N.useMemo(()=>h>0?se(l,h,g,r,s,A):[],[l,h,g,r,s,A]),o=N.useMemo(()=>O(l,g,s,A),[l,g,s,A]),f=s=>{m(a=>String(Math.max(0,(parseInt(String(a).replace(/[^\d]/g,""),10)||0)+s)))},b=s=>{z(a=>{const i=new Set(Array.isArray(a)?a:V.map(n=>n.id));return i.has(s)?i.delete(s):i.add(s),Array.from(i)})};
